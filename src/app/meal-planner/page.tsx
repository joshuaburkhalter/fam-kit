...
export default function MealPlannerPage() {
  ...
  const [currentWeekOffset, setCurrentWeekOffset] = useState(0); // 0 = this week
  ...
  // Export Recipe Ingredients to Grocery List
    ...
    <div className="max-w-7xl mx-auto p-4 sm:p-6 w-full space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Coordinate weekly family meals, link recipes, and export ingredients to your grocery list.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-2 rounded-xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white text-xs font-bold shadow-md shadow-emerald-600/20 transition-all hover:scale-102"
          >
      ...
      <div className="flex items-center justify-between p-3 rounded-2xl glass-panel border border-slate-800">
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white hover:bg-slate-800 text-xs font-semibold transition-colors"
        >
        ...
        <div className="text-center">
          ...
          {currentWeekOffset === 0 && (
            <span className="text-[10px] font-semibold text-emerald-400">Current Week</span>
          )}
        </div>
            ...
            return (
              <div
                ...
                className={`rounded-2xl border flex flex-col overflow-hidden transition-all ${
                  day.isToday
                    ? 'glass-panel border-emerald-500/50 shadow-lg shadow-emerald-950/30'
              ...
              >
                ...
                >
                  ...
                  <div className="text-[11px] text-slate-400 mt-0.5">{day.formatted}</div>
                </div>
                  ...
                  {mealSlots.map((slot) => {
                    ...
                    return (
                      <div
                        ...
                        className={`p-2.5 rounded-xl border transition-all cursor-pointer group flex flex-col justify-between min-h-[72px] ${
                          meal
                            ? 'bg-slate-900/90 border-slate-700/80 hover:border-emerald-500/60 text-slate-200'
                            : 'bg-slate-950/40 border-slate-900 hover:border-slate-700 hover:bg-slate-900/40 text-slate-500'
                        }`}
                      >
                        <div className="flex items-center justify-between text-[11px] font-semibold text-slate-400 mb-1">
                          ...
                          {!meal && <Plus className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity" />}
                        </div>

                        {meal ? (
                          <div className="space-y-1">
                            <div className="text-xs font-bold text-slate-100 line-clamp-2 group-hover:text-emerald-300">
                              {meal.title}
                            </div>
                            {meal.recipe && (
            ...
            <form onSubmit={handleSaveSlot} className="mt-4 space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Defrost chicken in morning, serve with penne"
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-2 text-xs text-white"
                />
              </div>

              {/* One-Tap Export Ingredients if linked recipe */}
              {slotRecipeId && (
                <div className="pt-1">
                  <button
                    type="button"
                    ...
                    className="w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-emerald-400 text-xs font-semibold transition-colors"
                  >
      ...
      {/* AI Brainstorm Weekly Meals Modal */}
      {showBrainstormModal && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
            ...
            <form onSubmit={handleAiBrainstorm} className="mt-4 space-y-4">
              <div>
                <label className="text-xs font-semibold text-slate-400 block mb-1.5">
                  Your Preferences & Dietary Requests
                </label>
                <textarea
                  value={aiPrompt}
                  onChange={(e) => setAiPrompt(e.target.value)}
                  rows={3}
                  className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-white focus:ring-2 focus:ring-emerald-500/50"
                  required
                />
              </div>
