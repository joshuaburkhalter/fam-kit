...
export const api = {
  ...
  getFeedbackRequests: async (): Promise<FeedbackRequest[]> => {
    ...
    return list.map((r) => ({
      ...
      submitted_by_user_id: r.submittedByUserId || r.submitted_by_user_id,
      submitted_by_user_name: r.submittedByUserName || r.submitted_by_user_name,
      submitted_by_user_email: r.submittedByUserEmail || r.submitted_by_user_email,
      household_id: r.householdId || r.household_id,
      household_name: r.householdName || r.household_name,
      admin_response: r.adminResponse || r.admin_response,
      admin_responded_at: r.adminRespondedAt || r.admin_responded_at,
      admin_responded_by: r.adminRespondedBy || r.admin_responded_by,
      upvotes: typeof r.upvotes === 'number' ? r.upvotes : 0,
      upvoters: Array.isArray(r.upvoters) ? r.upvoters : [],
      hasUpvoted: Boolean(r.hasUpvoted),
      created_at: r.createdAt || r.created_at,
  ...
  createFeedbackRequest: async (data: {
    ...
    return {
      ...
      admin_responded_at: r.adminRespondedAt || r.admin_responded_at,
      admin_responded_by: r.adminRespondedBy || r.admin_responded_by,
      upvotes: typeof r.upvotes === 'number' ? r.upvotes : 1,
      upvoters: Array.isArray(r.upvoters) ? r.upvoters : [],
      hasUpvoted: true,
      created_at: r.createdAt || r.created_at,
      updated_at: r.updatedAt || r.updated_at,
  ...
  upvoteFeedbackRequest: async (id: string): Promise<FeedbackRequest> => {
    const r = await fetchJson<any>('/feedback/vote', {
      method: 'POST',
