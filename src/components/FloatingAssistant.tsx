...
export const FloatingAssistant: React.FC = () => {
  ...
  const dockContainerRef = useFabAutoClose<HTMLDivElement>({
    isOpen: isExpanded,
    onClose: () => handleSmoothClose(false),
  });

  // Push history state so phone back button closes the expanded dock
  useEffect(() => {
    if (isExpanded) {
      if (!historyPushedRef.current) {
        historyPushedRef.current = true;
        closedByPopstateRef.current = false;
        window.history.pushState({ type: 'assistant_dock', timestamp: Date.now() }, '', window.location.href);
      }
